...
export default function ShopPage() {
  ...
  //   { value: 'citrus', label: t('shop.citrus') },
  //   { value: 'jagodičasto', label: t('shop.berries') },
  //   { value: 'egzotično', label: t('shop.exotic') },
  // ], [t])
  ...
  //   }
  //   if (sortBy === 'price-asc') {
  //     result.sort((a, b) => a.prices[0].price - b.prices[0].price)
  //   } else if (sortBy === 'price-desc') {
  //     result.sort((a, b) => b.prices[0].price - a.prices[0].price)
  //   }
  //   return result
      ...
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        ...
        {/* <div className="flex flex-col sm:flex-row gap-4 mb-10">
          <div className="flex-1 max-w-xs">
            <label htmlFor="category" className="sr-only">{t('shop.allCategories')}</label>
            <select
              id="category"
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="w-full px-4 py-3 bg-bg-page border border-border-light rounded text-sm text-text-nav font-sans focus:outline-none focus:border-text-nav-hover"
            >
              {categories.map((cat) => (
          ...
          <div className="flex-1 max-w-xs">
            <label htmlFor="sort" className="sr-only">{t('shop.sortPopular')}</label>
            <select
              id="sort"
              value={sortBy}
